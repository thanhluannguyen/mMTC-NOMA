...
noisePowerDensity = -104; % dBm/Hz
noiseVar = 10^(noisePowerDensity/10)*carrierFreq*1e6; % mW - Noise Variance

l_q = [Inf 75 50 50]; % meter - Distance Between Type-I MTCDs   
GI_Tx = 5; % dBi - Transmit Antenna Gain of Type-I MTCD
GI_Rx = 5; % dBi - Receive Antenna Gain of Type-I MTCD    
...
GII_Tx = 0; % dBi - Transmit Antenna Gain of Type-II MTCD
GII_Rx = 0; % dBi - Receive Antenna Gain of Type-II MTCD
PL_I2II = pathloss3GPP_UMi(GII_Rx,GII_Tx,carrierFreq, r_t);

lambda_t = (1e-2)*ones(1,M-1); % Type-II MTCDs/m^2 - Per Hop Type-II MTCDs Density
maxRateQoS = 1.5;
RateQoS = 0.5;
...
rho_t = [0,0.0,0.0,0]; % - Probability Energy Harvesting at [DI_1,...,DI_M]
eta_t = [0,0.9*ones(1,M-1)]; % - Energy Harvesting Efficiency

alpha_t= [0,0.2*ones(1,M-1)]; % - Energy Harvesting Ratio /BTEH
beta_t = [0,0.8*ones(1,M-1)]; % - Energy Harvesting Ratio /BPEH

% Quality-oriented multi-hop scheme (QoM)
[PA_QoMS,CPA_QoMS] = powCoeff(1);
R_M_TQoMS = log2(1+PA_QoMS(2)/CPA_QoMS(1))*(1-min(alpha_t(2:(M-2)).*(rho_t(2:(M-2))>0)))/(M-1)*RateQoS;
...
P_0= 10^(5/10); % dBm - Fixed Transmission Power
g_0= 10^(P_0/10)/noiseVar; % - Average Per Hop Fixed Transmission Power
...
%     AA(tt,isQoMS(tt,:)==1) = random('Burr',mu_t(tt),theta_t(tt),m_t(tt),[1,sum(isQoMS(tt,:)==1)]);
end