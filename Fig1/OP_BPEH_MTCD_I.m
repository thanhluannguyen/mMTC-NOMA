function out = OP_BPEH_MTCD_I(M,PL_I2I,r_t,PA_QoMS,CPA_QoMS,...
    rho_t,eta_t,beta_t,g_0,lambda_t,R_M_QoMS)
    ...
    for tt = 1:(M-1)
        ...
        a_2 = a_1/( 1-beta_t(tt+1) );
        a_3 = a_1/( 1-(a_1+1)*(1-PA_QoMS(2)) );
        a_4 = a_3/( 1-beta_t(tt+1) );
        
        SP_MTCDI_PQoM(tt+1) = (1-rho_t(tt+1))*iota_t(tt)*cCDF_X_t(rho_t,g_0,PL_I2I,Omega_t,tt+1,a_1)...
                              + (rho_t(tt+1))*iota_t(tt)*cCDF_X_t(rho_t,g_0,PL_I2I,Omega_t,tt+1,a_2);
        
        if (a_1 < PA_QoMS(2)/CPA_QoMS(1))
            SP_MTCDI_PQoM(tt+1) = SP_MTCDI_PQoM(tt+1)...
